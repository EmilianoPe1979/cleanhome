...
const CartServiceContext = createContext();
...
function cartReducer(state, action) {
  switch (action.type) {
    case "ADD_ITEM": // Acción para añadir un producto al carrito
      ...
      if (existingItem) {
        ...
        return {
          ...
          // Copiamos todo del payload (el servicio).
          //Le agregamos quantity: 1.
        };
      }

    case "UPDATE_QUANTITY":
      // Cambiar la cantidad personal de un servicio
      return {
        ...state,
        cartItems: state.cartItems.map(
          (item) =>
            item.id === action.payload.id
              ...
              : item
          // Si ambos id's coinciden, crea un nuevo objetivo con los datos actuales del servicio
    ...
    case "REMOVE_ITEM":
      ...
      // Se filtra el carrito para eliminar el servicio con el ID especifico
      // Devuelve todos los servicios, excepto el que tiene el id igual al que viene en action.payload
      return {
        ...state,
...
export function CartServiceProvider({ children }) {
  const [state, dispatch] = useReducer(cartReducer, initialState);

  // Acciones para modificar el carrito
  // payload service: Necesitamos todos los datos del servicio si este no se agrego al carrito todavia
  ...
  return (
    <CartServiceContext.Provider // Estado global del carrito y sus funciones
      value={{
        cartItems: state.cartItems, // lista de servicios en el carrito
        addItem, // función para añadir servicios
        updateQuantity, // función para actualizar cantidades
        clearCart, // función para limpiar el carrito
        removeItem, // función para eliminar un servicio del carrito
      }}
    >
      {children}
      {/* children Representa todos los componentes envueltos por el CartServiceProvider */}
    </CartServiceContext.Provider>
  );
}

// Hook personalizado para usar el carrito fácilmente en cualquier componente
export const useCart = () => useContext(CartServiceContext);
